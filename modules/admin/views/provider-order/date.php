...
$this->title = 'Заявка на поставку товаров на ' . date('d.m.Y', strtotime($date['end']));
            ...
            <?php foreach ($model as $k => $val): ?>
                ...
                <?php if ($rowspan == 1): ?>
                    <tr>
                        ...
                        <td><a href="<?= Url::to(['/admin/provider-order/detail', 'id' => $val['product_feature_id'], 'pid' => $orders[0]['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date['end']))]); ?>" style="text-decoration: underline;"><?= number_format($orders[0]['quantity']); ?></a></td>
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($orders as $y => $order): ?>
                        <?php if ($y != 0): ?>
                            <tr>
                                <td><?= $order['p_name']; ?></td>
                                <td><a href="<?= Url::to(['/admin/provider-order/detail', 'id' => $val['product_feature_id'], 'pid' => $order['p_id'], 'prid' => $val['provider_id'], 'date' => date('Y-m-d', strtotime($date['end']))]); ?>" style="text-decoration: underline;"><?= number_format($order['quantity']); ?></a></td>
